...
export default function SubmittedServicesCmp () {
  const { token , userType } = useAuthInfo()
  ...
  return (
    <section className='text-white/80 '>
      <div className=' w-full'>
        <div className='mb-5 flex flex-col gap-y-5 md:gap-y-0  md:flex-row md:justify-between md:items-center '>
          <button className=' text-sm md:text-base flex items-center gap-2 border rounded-full border-white/10 text-white/80 hover:text-white transition-all px-4 py-1  cursor-pointer'>
            <TbRosetteDiscountCheckFilled className='text-sm  text-[#a8ff57]' />
            Your Submitted Services
          </button>
          <Link href='/dashboard/submitted-services'>
            <button className=' text-sm md:text-base flex items-center gap-2 border rounded-full border-white/10 text-white/80 hover:text-white transition-all px-4 py-1  cursor-pointer'>
                    ...
                    {(() => {
                      ...
                      return (
                        <Link href={`dashboard/service-details/${data.id}`}>
                          <button className='text-nowrap border border-white/10 text-white/80 px-4 py-1.5 rounded-md hover:bg-[#a8ff57] hover:text-black transition cursor-pointer'>
                            View Details
                          </button>
                        </Link>
                      )
